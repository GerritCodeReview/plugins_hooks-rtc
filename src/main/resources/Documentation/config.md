...
setting `plugin.rtc.enabled` to `enforced`.

On the project info screen there is a dropdown list for the
`plugin.rtc.enabled` parameter which offers the values `true`,
...
`plugin.rtc.branch`. The branches may be configured using explicit
